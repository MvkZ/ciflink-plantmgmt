...
//				added by sunil - start
                if(data.pointIndex != undefined && data.pointIndex > 0)
                {
                	var tsFormat = d3.time.format('%b %-d, %Y %I:%M%p');
                	data.value = tsFormat(new Date(data.value));
                }
//				end
    ...
    function chart(selection) {
        selection.each(function(data) {
            ...
            var color_array = data.map(function(d,i) {
            	data[i].color = color(d, i);//added by sunil
            ...
            var interpolate1 = "linear";
        	var interpolate2 = "linear";
            if(data.length > 0)
            {
            	for(var dataIndex = 0;dataIndex<data.length;dataIndex++)
            	{
            		if(data[dataIndex].yAxis != undefined && data[dataIndex].yAxis == 1 && data[dataIndex].interpolate != undefined)
            			interpolate1 = data[dataIndex].interpolate;
            		if(data[dataIndex].yAxis != undefined && data[dataIndex].yAxis == 2 && data[dataIndex].interpolate != undefined)
            			interpolate2 = data[dataIndex].interpolate;
            	}
            }
...
//	             return d.interpolate;}).filter(function(d,i) 
//	            		 { 
...
//	            		  }))
            ...
            function mouseover_line(evt) {
                ...
                $.each(data, function(index, value){
                	...
                	var key  = "";
                	var flag = false;
                	var xVals = $.grep(value.values,function( n, i ) {
                		  return n.x == evt.value;
                	});
                	if(xVals.length > 0)
                		xVal = xVals[0]
                	else
                	{
                		flag = true;
                		xVals = $.grep(value.values,function( n, i ) {
                  		  return n.x < evt.value;
                		});
                		if(xVals.length > 0)
                	...
                	if(flag)
                	{
                		if(xVal == undefined || xVal == null)
                			key = value.key +":No Data";
                		else
                			key = value.key +"@"+ tsFormat(new Date(xVal.x));
                	}
                	else
                		key = value.key;
                	if(value.disabled == false || value.disabled == undefined)
                	{
                		if(xVal != undefined && xVal != null)
		            		evt.series.push(
		        				{
