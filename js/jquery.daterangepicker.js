...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text: 'Month to date', dateStart: function(){ return Date.parseRange('today').moveToFirstDayOfMonth();  }, dateEnd: 'today' },
			{text: 'Year to date', dateStart: function(){ var x= Date.parseRange('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
			{text: 'The previous Month', dateStart: function(){ return Date.parseRange('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parseRange('1 month ago').moveToLastDayOfMonth();  } }
		...
		prevLinkText: 'Prev',
		doneButtonText: 'Done',
		earliestDate: Date.parseRange('-15years'), //earliest date allowed 
		latestDate: Date.parseRange('+15years'), //latest date allowed 
	...
	//key press event for the input to check if the alphabets are entered
...
//		}
	...
	var inputDateA, inputDateB = Date.parseRange('today');
	var inputDateAtemp, inputDateBtemp;
	if(rangeInput.size() == 2){
		inputDateAtemp = Date.parseRange( rangeInput.eq(0).val() );
		inputDateBtemp = Date.parseRange( rangeInput.eq(1).val() );
		if(inputDateAtemp == null){inputDateAtemp = inputDateBtemp;} 
		if(inputDateBtemp == null){inputDateBtemp = inputDateAtemp;} 
	}
	else {
		inputDateAtemp = Date.parseRange( rangeInput.val().split(options.rangeSplitter)[0] );
		inputDateBtemp = Date.parseRange( rangeInput.val().split(options.rangeSplitter)[1] );
	...
	function showRP(){
		...
		//hiding all the open date range pickers(if open)
		var rangePickersJq = $(".ui-daterangepicker",$(".ui-daterangepickercontain"));
		for(var i=0;i<rangePickersJq.length;i++)
		{
			$(rangePickersJq[i]).hide();
			$(rangePickersJq[i]).data('state','closed');
		}
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			//custom date range
				...
				var dateStart = (typeof jQuery(this).data('dateStart') == 'string') ? Date.parseRange(jQuery(this).data('dateStart')) : jQuery(this).data('dateStart')();
				var dateEnd = (typeof jQuery(this).data('dateEnd') == 'string') ? Date.parseRange(jQuery(this).data('dateEnd')) : jQuery(this).data('dateEnd')();
	...
	.appendTo(rpPickers);
//	var cancelBtn = jQuery('<button class="btnCancel ui-state-default ui-corner-all">Close</button>')
//	.click(function(){
//		hideRP();
//	})
//	.appendTo(rpPickers);
...
return g._start.call({},s);};}());Date._parse=Date.parse;Date.parseRange=function(s){var r=null;if(!s){return null;}
...
var TimeSpan = function (days, hours, minutes, seconds, milliseconds) {
    ...
    this.getTotalMatilliseconds = function () {
